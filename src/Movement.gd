extends TileMap

var chessmen_list = Array()
var chessmen_coords = Dictionary()

var jumped_over_tiles = Dictionary()
var passable_tiles = Dictionary()

var kings = Dictionary()

var Mapping setget setMapping

var initial_pawn_tiles_white
var initial_pawn_tiles_black

var board_tiles

func setMapping(mapping_node):
	Mapping = mapping_node
	initial_pawn_tiles_white = Mapping.black_pawn_tiles
	initial_pawn_tiles_black = Mapping.white_pawn_tiles

	board_tiles = Mapping.regular_hexagon(0, 0)

func check_possible_moves(NPC):
	var initial_position = NPC.tile_position
	var king = kings[NPC.color]
	var chessmen_coords_copy = chessmen_coords.duplicate()
	
	var king_threatening_pieces = Array()
	
	if NPC != king:
		chessmen_coords.erase(NPC.tile_position)

	for tile_piece in chessmen_coords:
		var piece = chessmen_coords[tile_piece]
		
		if piece.color != NPC.color and piece.type != "King"\
		and king.tile_position in find_possible_moves(piece):
			
			king_threatening_pieces.append(piece)
			
			if len(king_threatening_pieces)==2 and NPC!=king: 
				chessmen_coords = chessmen_coords_copy
				return []

	chessmen_coords = chessmen_coords_copy
		
	var range_of_movement = find_possible_moves(NPC)

	if not king_threatening_pieces and NPC != king:
		return range_of_movement
		
	else:
		var chessmen_list_copy = chessmen_list.duplicate()
		
		for tile in range_of_movement.duplicate():
			var erased_piece
			
			if tile in chessmen_coords:
				chessmen_list_copy.erase(chessmen_coords[tile])
				erased_piece = chessmen_coords[tile]
			
			NPC.tile_position = tile
			chessmen_coords = find_chessmen_coords(chessmen_list_copy)
					
			if NPC == king and erased_piece:
				for tile_piece in chessmen_coords:
					if chessmen_coords[tile_piece].color != NPC.color\
					and king.tile_position in find_possible_moves(chessmen_coords[tile_piece]):
						range_of_movement.erase(tile)
			
			else:
				for tp in king_threatening_pieces:
					if tp!=erased_piece\
					and king.tile_position in find_possible_moves(tp):
						range_of_movement.erase(tile)
			
			chessmen_coords = chessmen_coords_copy
		
		NPC.tile_position = initial_position
		
		return range_of_movement

func find_chessmen_coords(chessmen_list_local = chessmen_list):
	var coords_dict = Dictionary()
	
	for NPC in chessmen_list_local:
		coords_dict[NPC.tile_position] = NPC
		
	return coords_dict

func find_possible_moves(NPC):
	var range_of_movement = Array()
	var position = NPC.tile_position
	
	if "Pawn" == NPC.type:
		range_of_movement = pawn_movement(NPC, position)
		
	elif "Knight" == NPC.type:
		range_of_movement = knight_movement(NPC, position)
		
	elif "Bishop" == NPC.type:
		range_of_movement = bishop_movement(NPC, position)
	
	elif "Rook" == NPC.type:
		range_of_movement = rook_movement(NPC, position)
		
	elif "Queen" == NPC.type:
		range_of_movement = bishop_movement(NPC, position) + rook_movement(NPC, position)
	
	elif "King" == NPC.type:
		range_of_movement = king_movement(NPC, position)
	
	return range_of_movement

func rook_movement(NPC, position, iterations = 12, check = true):
	var result = []
	var diagonals = [[1, 1], [1, -1], [-1, 1], [-1, -1]]
	
	for diagonal in diagonals:
		var array_to_check = Mapping.draw_diagonal_line(position, iterations-1, diagonal[0], diagonal[1]).slice(1, -1)
		
		if check:
			result.append_array(check_array(array_to_check, NPC))

		else:
			result.append_array(array_to_check)
	
	var vertical_1 = Mapping.draw_vertical_line(position, iterations).slice(1, -1)
	var vertical_2 = Mapping.draw_vertical_line(position, iterations, -1).slice(1, -1)
	
	if check:
		result.append_array(
			check_array(vertical_1, NPC) + check_array(vertical_2, NPC))
			
	else:
		result.append_array(vertical_1 + vertical_2)
	
	return result

func bishop_movement(NPC, position, iterations = 5, check = true):
	var result = []
	var horizontal1 = []
	var horizontal2 = []
	
	for i in iterations+1:
		if i == 0: continue
		horizontal1.append(Vector2(position[0]+i*2, position[1]))
		horizontal2.append(Vector2(position[0]-i*2, position[1]))
	
	if check:
		result.append_array(
			check_array(horizontal1, NPC)+check_array(horizontal2, NPC))
	else:
		result.append_array(horizontal1 + horizontal2)
		
	var diagonals = [[2, 1, 1], [-1, -2, 1], [2, 1, -1], [-1, -2, -1]]
	
	for diagonal in diagonals:
		var array_to_check = Mapping.bishop_diagonal(position, diagonal[0], diagonal[1], diagonal[2], iterations)
		
		if check:
			result.append_array(check_array(array_to_check, NPC))
		else:
			result.append_array(array_to_check)
			
	return result

func knight_movement(piece, position):
	var result = []
	var moves
	
	if int(position[0])%2==0:
		moves = [[-1, -2], [-2, -2], [1, -2], [2, -2], [3, 0], [3, 1],
		[2, 2], [1, 3], [-1, 3], [-2, 2], [-3, 1], [-3, 0]]
	else:
		moves = [[1, -3], [2, -2], [-1, -3], [-2, -2], [3, -1],
		[3, 0], [-3, -1], [-3, 0], [-2, 2], [-1, 2], [1, 2], [2, 2]]
		
	for move in moves:
		var tile =(Vector2(position[0]-move[0], position[1]-move[1]))
		if check_movement(tile) or tile in chessmen_coords and chessmen_coords[tile].color != piece.color:
			result.append(tile)
	
	return result

func pawn_movement(pawn, position):
	var array_to_check
	
	if pawn.color == 'black' and position in initial_pawn_tiles_black:
		array_to_check = Mapping.draw_vertical_line(position, 3)
	
	elif pawn.color == 'black':
		array_to_check = Mapping.draw_vertical_line(position, 2)
	
	elif pawn.color == 'white' and position in initial_pawn_tiles_white:
		array_to_check = Mapping.draw_vertical_line(position, 3, -1)
	
	else:
		array_to_check = Mapping.draw_vertical_line(position, 2, -1)
	
	var result = check_array(array_to_check.slice(1, -1))
	
	if result.size() == 2:
		passable_tiles[pawn] = result[0]
	
	return result + pawn_attack(pawn, position)

func pawn_attack(pawn, position, check = true):
	var result = []
	var closest_tiles = Mapping.find_closest_tiles(position)
	var attack_tiles = []
	
	if pawn.color == 'white' and Mapping.chess_type == 'glinski':
		if int(position[0])%2!=0:
			attack_tiles.append_array([closest_tiles[3], closest_tiles[1]])
		else:
			attack_tiles.append_array([closest_tiles[0], closest_tiles[4]])
	
	elif pawn.color == 'black' and Mapping.chess_type == 'glinski':
		if int(position[0])%2!=0:
			attack_tiles.append_array([closest_tiles[0], closest_tiles[4]])
		else:
			attack_tiles.append_array([closest_tiles[3], closest_tiles[1]])
	
	elif pawn.color == 'black':
		attack_tiles.append_array(
			Mapping.bishop_diagonal(position, 2, 1, 1, 1) + Mapping.bishop_diagonal(position, 2, 1, -1, 1))
	
	elif pawn.color == 'white':
		attack_tiles.append_array(
			Mapping.bishop_diagonal(position, -1, -2, 1, 1) + Mapping.bishop_diagonal(position, -1, -2, -1, 1))
	
	if check == true:
		for tile in attack_tiles:
			if tile in chessmen_coords:
				var piece = chessmen_coords[tile]
				if piece.color != pawn.color:
					result.append(tile)
					
			elif tile in jumped_over_tiles:
				result.append(tile)
	else:
		result = attack_tiles

	return result
	
func king_movement(king, position):
	var output = rook_movement(king, position, 2) + bishop_movement(king, position, 1)

	for piece in chessmen_list:
		var array_to_check
		
		if piece.color != king.color:
			if not 'Pawn' == piece.type and not 'King' == piece.type:
				array_to_check = find_possible_moves(piece)
				
			elif 'King' == piece.type:
				array_to_check = rook_movement(piece, piece.tile_position, 2, false)\
					+ bishop_movement(piece, piece.tile_position, 1, false)
					
			elif 'Pawn' == piece.type:
				array_to_check = pawn_attack(piece, piece.tile_position, false)
						
			output = remove_intersection(output, array_to_check)
			
	return output

func remove_intersection(array_to_clean, array_to_check):
	for tile in array_to_clean.duplicate():
		if tile in array_to_check:
			array_to_clean.erase(tile)
			
	return array_to_clean

func check_array(tile_array, piece = null):
	var result = Array()
	
	for tile in tile_array:
		if check_movement(tile):
			result.append(tile)
			
		elif piece and tile in chessmen_coords and piece.color != chessmen_coords[tile].color:
			result.append(tile)
			break
			
		else:
			break
			
	return result
	
func check_movement(new_position):
	return new_position in board_tiles and not new_position in chessmen_coords
